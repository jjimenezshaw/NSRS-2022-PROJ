...
def process_ggxf_to_multipage_tiff(in_ggxf, out_tiff, do_scale):
    ...
    for i, layer_info in enumerate(target_layers):
        ...
        if do_scale:
            ...
            data_int16 = np.where(np.isnan(data), nodata_val, data_scaled).astype(
                np.int16
            )
            print(offset, scale)
            print(vmax, vmin)
        ...
        options = ["COMPRESS=DEFLATE", f"PREDICTOR={2 if do_scale else 3}", "TILED=YES"]
        if i > 0:
        ...
        out_ds = driver.Create(
            ...
            n_rows,
            1,
            gdal.GDT_Int16 if do_scale else gdal.GDT_Float32,
            options=options,
        )
        ...
        out_ds.SetMetadataItem(
            ...
            f"ITRF2020 (EPSG:9990) to NAPGD2022. Converted from {in_ggxf} (last modified at {today})",
        )
        out_ds.SetMetadataItem("TIFFTAG_DATETIME", f"{now}")
        out_ds.SetMetadataItem(
            "TIFFTAG_COPYRIGHT", "Derived from work by NGS/NOAA. CC-BY 4.0."
        )
        out_ds.SetMetadataItem("target_crs_epsg_code", "*NAPGD2022*")
        out_ds.SetMetadataItem(
            "TYPE",
            "VELOCITY" if is_velocity else "VERTICAL_OFFSET_GEOGRAPHIC_TO_VERTICAL",
        )
